...
TruckScale is different: it stores weighbridge data in MySQL. WS-Sale-App reads completed weighing and reference records, matches them to SO/vehicle data, and feeds operational results back into the workflow. Completed-weigh source tables such as `tblscale` remain read-only; the only current controlled write boundary is INSERT/DELETE on `tbl_keyone` for the pre-weigh queue lifecycle.
...
- Automated E2E รอบ 2026-07-22 ผ่าน 10/10: comprehensive multi-bill verification gate, UAT full loop 5 stages และ role navigation 4 roles; evidence เป็น `PASSED_COMPLETE` และผูก hash กับ source/test ปัจจุบัน ขณะที่ business UAT sign-off, security/performance test และ TruckScale-connected integration ยังเป็นหลักฐานแยก.
- Full steps and manual retest checklist are documented in `docs/09-AUTOMATED-QA.md`.
