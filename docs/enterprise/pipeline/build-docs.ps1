...
param(
  ...
  # หา repo root แบบ dynamic — ห้าม hardcode path ของเครื่องใดเครื่องหนึ่ง
  # ลำดับ: git rev-parse -> ไต่ขึ้นจาก docs/enterprise/pipeline (3 ชั้น)
  [string]$SourceRepo     = $(
    $r = & git -C $PSScriptRoot rev-parse --show-toplevel 2>$null
    if ($LASTEXITCODE -eq 0 -and $r) { (Resolve-Path $r).Path }
