...
function migrationSummary(files, policy = {}) {
  const migrations = files.filter(file => file.group === 'database-migrations').map(file => {
    ...
    // แต่ละโฟลเดอร์คือบัญชี migration คนละเล่ม — backend/migrations เป็นของ SQL Server
    // (wf.SchemaMigration) ส่วน backend/migrations/mysql เป็นของฐานเครื่องชั่ง
    // (wf_schema_migration) เลขลำดับจึงเดินคู่ขนานกันได้ ไม่ใช่เลขซ้ำ
    return { path: file.path, ledger: path.dirname(file.path), sequence: match ? Number(match[1]) : null };
  });
  const sequenced = migrations.filter(item => item.sequence !== null);
  const grouped = sequenced.reduce((acc, item) => {
    const key = item.ledger + '#' + item.sequence;
    acc[key] = acc[key] || [];
  ...
  const duplicates = Object.entries(grouped)
    ...
    .map(([key, paths]) => ({ sequence: Number(key.split('#').pop()), paths }));
...
function migrationIsApplied(relativePath, policy = {}) {
  const base = path.basename(relativePath);
  if ((policy.excludedFiles || []).includes(base)) return false;
  return !(policy.excludedPatterns || []).some(pattern => new RegExp(pattern).test(base));
}

// \u0E2A\u0E23\u0E49\u0E32\u0E07\u0E2A\u0E16\u0E32\u0E19\u0E30 schema wf \u0E08\u0E32\u0E01 migration SQL \u0E15\u0E32\u0E21\u0E25\u0E33\u0E14\u0E31\u0E1A\u0E17\u0E35\u0E48 run_migrations.js \u0E23\u0E31\u0E19\u0E08\u0E23\u0E34\u0E07
// \u0E15\u0E49\u0E2D\u0E07 replay \u0E17\u0E31\u0E49\u0E07 CREATE \u0E41\u0E25\u0E30 DROP \u0E44\u0E21\u0E48\u0E43\u0E0A\u0E48\u0E19\u0E31\u0E1A\u0E40\u0E09\u0E1E\u0E32\u0E30 CREATE \u0E21\u0E34\u0E09\u0E30\u0E19\u0E31\u0E49\u0E19\u0E15\u0E32\u0E23\u0E32\u0E07\u0E17\u0E35\u0E48\u0E16\u0E39\u0E01 drop
// \u0E20\u0E32\u0E22\u0E2B\u0E25\u0E31\u0E07 (\u0E40\u0E0A\u0E48\u0E19 wf.GiveawayIssue \u0E43\u0E19 002) \u0E08\u0E30\u0E16\u0E39\u0E01\u0E19\u0E31\u0E1A\u0E40\u0E01\u0E34\u0E19 \u0E41\u0E25\u0E30\u0E40\u0E25\u0E02\u0E44\u0E21\u0E48\u0E15\u0E23\u0E07 sys.tables
function extractWfSchemaObjects(files, policy = {}, read = readSource) {
  const migrations = files
    .filter(file => file.group === 'database-migrations' && migrationIsApplied(file.path, policy))
    .map(file => file.path)
    .sort((a, b) => path.basename(a).localeCompare(path.basename(b), 'en'));
  const buckets = { TABLE: new Map(), VIEW: new Map() };
  const createExpression = /\bCREATE\s+(?:OR\s+ALTER\s+)?(TABLE|VIEW)\s+(?:IF\s+NOT\s+EXISTS\s+)?\[?wf\]?\s*\.\s*\[?(\w+)/gi;
  const selectIntoExpression = /\bSELECT\b[\s\S]{0,400}?\bINTO\s+\[?wf\]?\s*\.\s*\[?(\w+)/gi;
  const dropExpression = /\bDROP\s+(TABLE|VIEW)\s+(?:IF\s+EXISTS\s+)?\[?wf\]?\s*\.\s*\[?(\w+)/gi;
  for (const relativePath of migrations) {
    const text = read(relativePath);
    const events = [];
    for (const match of text.matchAll(createExpression)) events.push({ at: match.index, kind: match[1].toUpperCase(), name: match[2], dropped: false });
    for (const match of text.matchAll(selectIntoExpression)) events.push({ at: match.index, kind: 'TABLE', name: match[1], dropped: false });
    for (const match of text.matchAll(dropExpression)) events.push({ at: match.index, kind: match[1].toUpperCase(), name: match[2], dropped: true });
    events.sort((a, b) => a.at - b.at);
    for (const event of events) {
      const bucket = buckets[event.kind];
      const key = event.name.toLowerCase();
      if (event.dropped) bucket.delete(key);
      else if (!bucket.has(key)) bucket.set(key, { name: event.name, origin: relativePath });
    }
  }
  const byName = (a, b) => a.name.localeCompare(b.name, 'en');
  return {
    tables: [...buckets.TABLE.values()].sort(byName),
    views: [...buckets.VIEW.values()].sort(byName),
...
function scanOnce(config = loadConfig()) {
  ...
  const wfSchema = extractWfSchemaObjects(sourceFiles, migrationPolicy);
  ...
  return {
    ...
    summary: {
      ...
      appliedMigrationCount: wfSchema.appliedMigrationCount,
      wfTableCount: wfSchema.tables.length,
      wfViewCount: wfSchema.views.length,
...
function buildAlignmentReport(source, docs, config, accepted, drift) {
  ...
  // sourceInventorySha256 ใน front matter = คำยืนยันของมนุษย์ว่า "เอกสารนี้ถูก align กับ source ชุดนี้"
  // จึงไม่เขียนทับอัตโนมัติ แต่ต้องเตือนเมื่อไม่ตรง มิฉะนั้นคำยืนยันจะค้างอยู่โดยไม่มีใครรู้
  for (const doc of docs.documents) {
    const declared = frontMatterValue(documentText(doc), 'sourceInventorySha256');
    if (!declared) continue;
    if (declared.toUpperCase() !== source.sourceInventorySha256.toUpperCase()) {
      addGap(gaps, 'WARNING', 'STALE_SOURCE_INVENTORY_CLAIM', doc.documentId + ' declares sourceInventorySha256 ' + declared.slice(0, 8) + '…, while source is ' + source.sourceInventorySha256.slice(0, 8) + '…; re-review then update the claim.', { documentId: doc.documentId, path: doc.path });
    }
  }

  // เอกสารที่ประกาศ GENERATED:SOURCE-FACTS ต้องตรงกับ source เสมอ
  // มิฉะนั้นตัวเลขจะเงียบๆ ล้าสมัยเหมือนที่เคยเกิดกับ Source snapshot ที่พิมพ์มือ
  for (const relative of listDocumentsWithMarker(FACTS_BLOCK_START)) {
    const file = path.join(ENTERPRISE_ROOT, ...relative.split('/'));
    const text = fs.readFileSync(file, 'utf8').replace(/^﻿/, '').replace(/\r\n?/g, '\n');
    const start = text.indexOf(FACTS_BLOCK_START);
    const end = text.indexOf(FACTS_BLOCK_END);
    if (end < start) {
      addGap(gaps, 'ERROR', 'SOURCE_FACTS_BLOCK_INVALID', relative + ' has a GENERATED:SOURCE-FACTS start marker without a matching end marker.', { path: relative });
      continue;
    }
    if (text.slice(start, end + FACTS_BLOCK_END.length) !== renderSourceFactsBlock(source, relative)) {
      addGap(gaps, 'WARNING', 'SOURCE_FACTS_BLOCK_STALE', relative + ' has a GENERATED:SOURCE-FACTS block that no longer matches source; run source.ps1 sync-facts.', { path: relative });
    }
  }
...
const FACTS_BLOCK_START = '<!-- BEGIN GENERATED:SOURCE-FACTS -->';
const FACTS_BLOCK_END = '<!-- END GENERATED:SOURCE-FACTS -->';

function runtimeVersionLabel(source) {
  const versions = (source.summary.packageVersions || []).map(item => item.version).filter(Boolean);
  const distinct = [...new Set(versions)];
  if (!distinct.length) return 'ไม่ทราบ';
  return distinct.length === 1 ? distinct[0] : distinct.join(' / ') + ' (ไม่ตรงกัน)';
}

// ตั้งใจไม่ใส่ git commit เพราะจะทำให้ block "ล้าสมัย" ทุก commit แม้ source ไม่เปลี่ยน
// จน preflight -Strict บล็อกวนไม่จบ; source inventory SHA ระบุสถานะ source ได้ตรงกว่า
// ส่วน commit ของแต่ละ baseline บันทึกอยู่ใน pipeline/baselines/history/ แล้ว
function sourceFactRows(source) {
  const summary = source.summary;
  const wf = source.wfSchema || { tables: [], views: [] };
  const ledger = summary.appliedMigrationCount;
  const excluded = summary.migrationFileCount - ledger;
  return [
    ['Runtime version', '`' + runtimeVersionLabel(source) + '`'],
    ['Source inventory SHA256', '`' + source.sourceInventorySha256 + '`'],
    ['Tracked source files', String(summary.fileCount)],
    ['Backend API', summary.routeMountCount + ' route mounts / ' + summary.endpointCount + ' endpoints'],
    ['Frontend', summary.portalCount + ' portal keys / ' + summary.roleCount + ' roles'],
    ['Roles', source.roles.map(role => '`' + role + '`').join(' · ') || '-'],
    ['Migrations', summary.migrationFileCount + ' ไฟล์ (ledger ' + ledger + ' — ยกเว้นตามนโยบาย ' + excluded + ') · ล่าสุด ' + String(summary.latestMigrationSequence).padStart(3, '0')],
    ['Schema `wf`', wf.tables.length + ' ตาราง / ' + wf.views.length + ' views'],
  ];
}

function renderSourceFactsBlock(source, documentRelativePath = '') {
  const tick = String.fromCharCode(96);
  const depth = Math.max(documentRelativePath.split('/').length - 1, 0);
  const command = '../'.repeat(depth) + 'pipeline/source.ps1';
  const lines = [
    FACTS_BLOCK_START,
    '> ตัวเลขในตารางนี้สร้างจาก source โดยอัตโนมัติ ห้ามแก้ด้วยมือ; ให้รัน ' + tick + command + ' sync-facts' + tick + ' หลังโค้ดเปลี่ยน', '',
    '| ข้อเท็จจริงจาก source | ค่า |',
    '|---|---|',
    ...sourceFactRows(source).map(([label, value]) => '| ' + escapeCell(label) + ' | ' + escapeCell(value) + ' |'),
    '',
    FACTS_BLOCK_END,
  ];
  return lines.join('\n');
...
// ค้นหาเอกสารที่ประกาศ block เอง แทนการ hardcode รายชื่อไฟล์ไว้ในโค้ด
// เพิ่ม block ในเอกสารใหม่ได้โดยไม่ต้องแก้ pipeline
function listDocumentsWithMarker(marker) {
  const excludedPrefixes = (DOC_CONFIG.excludePrefixes || []).map(prefix => slash(prefix));
  const matches = [];
  const walk = directory => {
    const entries = fs.readdirSync(directory, { withFileTypes: true }).sort((a, b) => a.name.localeCompare(b.name, 'en'));
    for (const entry of entries) {
      const full = path.join(directory, entry.name);
      const relative = slash(path.relative(ENTERPRISE_ROOT, full));
      ...
      if (fs.readFileSync(full, 'utf8').includes(marker)) matches.push(relative);
    }
  };
  walk(ENTERPRISE_ROOT);
  return matches;
}

function syncSourceFacts(source) {
  if (!source.scanStable) throw new Error('Source changed during scan; source facts were not modified.');
  const targets = listDocumentsWithMarker(FACTS_BLOCK_START);
  if (!targets.length) {
    console.log('No document declares a GENERATED:SOURCE-FACTS block.');
    return false;
  }
  let written = 0;
  for (const relative of targets) {
    const file = path.join(ENTERPRISE_ROOT, ...relative.split('/'));
    const before = fs.readFileSync(file, 'utf8').replace(/^﻿/, '').replace(/\r\n?/g, '\n');
    const after = replaceGeneratedBlock(before, FACTS_BLOCK_START, FACTS_BLOCK_END, renderSourceFactsBlock(source, relative));
    if (after === before) continue;
    atomicWrite(file, after);
    written++;
    console.log('  updated ' + relative);
  }
  console.log('Source facts: ' + written + ' of ' + targets.length + ' document(s) updated from ' + source.sourceInventorySha256 + '.');
  return written > 0;
}
...
function printSummary(analysis) {
  ...
  console.log(`  wf schema         : ${analysis.source.summary.wfTableCount} tables / ${analysis.source.summary.wfViewCount} views`);
...
function main() {
  try {
    ...
    const supported = new Set(['scan', 'impact', 'validate', 'preflight', 'status', 'accept', 'release-check', 'sync-api', 'sync-facts']);
    if (!supported.has(options.command)) throw new Error('Unsupported command: ' + options.command);
    let analysis = analyse();
    printSummary(analysis);
    if (options.command === 'sync-api' || options.command === 'sync-facts') {
      const label = options.command === 'sync-api' ? 'API Reference' : 'Source facts';
      if (options.noWrite) console.log('No-write: ' + label + ' was not modified.');
      else if (options.command === 'sync-api') syncApiReference(analysis.source);
      else syncSourceFacts(analysis.source);
      analysis = analyse();
