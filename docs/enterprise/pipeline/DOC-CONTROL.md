...
# rewrite every GENERATED:SOURCE-FACTS block (counts, versions, wf schema) from source
./source.ps1 sync-facts
...
- `docs.ps1 sync-index` เขียน block `GENERATED:DOCUMENT-INDEX` ใน `DOCUMENT-INDEX.md` จากรายการเอกสารจริง — สารบัญจึงไม่ล้าสมัยเมื่อเพิ่มหรือย้ายไฟล์ และไม่มีรายชื่อไฟล์ hardcode ในโค้ด
- `source.ps1 sync-facts` เขียนทุก block `GENERATED:SOURCE-FACTS` (runtime version, commit, source SHA, จำนวนไฟล์/mount/endpoint/portal/role/migration และจำนวนวัตถุใน schema `wf`); คำสั่ง**ค้นหาเอกสารที่ประกาศ block เอง** จึงเพิ่ม block ในเอกสารใหม่ได้โดยไม่ต้องแก้โค้ด pipeline — ห้ามพิมพ์ตัวเลขเหล่านี้ลง prose ซ้ำ เพราะจะไม่มีอะไร sync ให้
- `preflight` เตือนเองเมื่อ block ล้าสมัย (`SOURCE_FACTS_BLOCK_STALE`) หรือเมื่อ front matter `sourceInventorySha256` ไม่ตรง source ปัจจุบัน (`STALE_SOURCE_INVENTORY_CLAIM`); ค่าใน front matter คือ**คำยืนยันของมนุษย์**ว่าอ่าน source diff แล้ว จึงไม่ถูกเขียนอัตโนมัติ ต้อง review ก่อนแก้เอง
