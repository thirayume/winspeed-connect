...
# 1) python deps
Say "`n[1/7] Ensure python deps (python-docx, mistune)..." Yellow
...
Say "[2/7] Ensure node deps (pptxgenjs, mermaid-cli) in cache..." Yellow
...
if (-not $SkipDiagrams) {
  Say "[3/7] Diagrams: author + render mermaid..." Yellow
...
} else { Say "[3/7] Diagrams skipped" }

# 4) UAT: sync คลังเคสจาก uat-cases.json ลงมาร์กดาวน์ก่อน แล้ว DOCX จึงจะได้ตารางล่าสุด
Say "[4/7] Sync UAT case catalogue + build XLSX workbooks..." Yellow
node (Join-Path $HERE "sync_uat_markdown.js")
node (Join-Path $HERE "build_xlsx.js")

# 5) DOCX matrix
Say "[5/7] Build DOCX matrix (SRS, Tech Spec, User Guides)..." Yellow
$env:PYTHONUTF8="1"; & $pythonResolved @pythonPrefix (Join-Path $HERE "build_docx.py")

# 5) PPTX matrix
Say "[6/7] Build PPTX matrix (training + support)..." Yellow
node (Join-Path $HERE "build_pptx.js")

# 6) DRAWIO (รูปทรงแก้ไขได้ ไม่ใช่ภาพฝัง) แล้วตรวจว่าไม่มีเนื้อหาหายระหว่างแปลง
Say "[7/7] Build DRAWIO (editable shapes) + verify..." Yellow
$PIPE = Split-Path $HERE -Parent
node (Join-Path $PIPE "build-drawio.js")
node (Join-Path $PIPE "check-diagrams.js")
if ($LASTEXITCODE -ne 0) { Say "  ไดอะแกรมไม่ผ่านการตรวจ - แก้ mermaid-to-mxgraph.js ก่อนส่งมอบ" Red; exit 1 }

Say "`n== DONE ==" Green
Say "DOCX   -> $ENT\01-DOCX\generated"
Say "PPTX   -> $ENT\02-PPTX\generated"
Say "DRAWIO -> $ENT\03-DRAWIO\worldfert-all-diagrams.drawio"
Say "XLSX   -> $ENT\06-XLSX\uat"
