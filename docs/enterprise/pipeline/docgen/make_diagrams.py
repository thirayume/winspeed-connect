...
# ────────────────────────────────────────────────────────────────
# ไฟล์ .mmd ใน pipeline/diagrams คือ "ต้นฉบับ" (แก้ที่ไฟล์ .mmd ได้เลย)
# สคริปต์นี้ทำหน้าที่ seed เฉพาะไฟล์ที่ยังไม่มี — ไม่เขียนทับของเดิม
# เพราะการเขียนทับจะทำให้แก้ diagram ต้องมาแก้โค้ด python (hardcode)
#
# บังคับสร้างใหม่จาก template ในไฟล์นี้:  WF_DIAGRAMS_FORCE=1
# ────────────────────────────────────────────────────────────────
force = os.environ.get("WF_DIAGRAMS_FORCE") == "1"
seeded, kept = [], []
for name, content in diagrams.items():
    target = os.path.join(D, name + ".mmd")
...
print(f"diagrams: kept {len(kept)} existing .mmd (source of truth), seeded {len(seeded)} -> {D}")
for n in seeded: print(" + seeded", n)
if force and seeded: print(" (WF_DIAGRAMS_FORCE=1 — เขียนทับจาก template)")
