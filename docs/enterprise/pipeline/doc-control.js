...
function validateInventory(inventory, config, accepted) {
  ...
  for (const doc of inventory.documents) {
    ...
    // ตรวจลิงก์เฉพาะเนื้อความ ไม่ตรวจในบล็อกโค้ด
    //
    // SQL อย่าง `INSERT INTO [SMID]([Name],[Prefix],[Type])` เข้ารูป [..](..) พอดี
    // จึงถูกนับเป็นลิงก์เสียทั้งที่อยู่ใน ```sql ที่กั้นไว้แล้ว
    // แทนที่เนื้อในบล็อกด้วยบรรทัดว่างจำนวนเท่าเดิม เพื่อให้เลขบรรทัดที่รายงานยังตรง
    const linkScanText = text.replace(/```[\s\S]*?```|~~~[\s\S]*?~~~/g,
      block => block.replace(/[^\n]/g, ' '));

    const linkRegex = /!?\[[^\]]*\]\(([^)]+)\)/g;
    let match;
    while ((match = linkRegex.exec(linkScanText)) !== null) {
      ...
      if (!fs.existsSync(resolved)) {
        add('ERROR', 'BROKEN_LOCAL_LINK', doc, `Local link target does not exist: ${match[1]}`, lineNumberAt(linkScanText, match.index));
      }
    }
...
const INDEX_BLOCK_START = '<!-- BEGIN GENERATED:DOCUMENT-INDEX -->';
const INDEX_BLOCK_END = '<!-- END GENERATED:DOCUMENT-INDEX -->';

function escapeIndexCell(value) {
  return String(value == null ? '' : value).replace(/\|/g, '\\|').replace(/\r?\n/g, ' ').trim();
}

// สร้างสารบัญจาก inventory จริง ไม่ให้ใครต้องมาไล่แก้มือทุกครั้งที่เพิ่ม/ย้ายเอกสาร
function renderDocumentIndexBlock(inventory) {
  const tick = String.fromCharCode(96);
  const documents = [...inventory.documents].sort((a, b) => a.path.localeCompare(b.path, 'en'));
  const byFolder = new Map();
  for (const doc of documents) {
    const parts = doc.path.split('/');
    const folder = parts.length > 1 ? parts[0] : '(ราก)';
    if (!byFolder.has(folder)) byFolder.set(folder, []);
    byFolder.get(folder).push(doc);
  }
  const lines = [
    INDEX_BLOCK_START,
    '> สารบัญนี้สร้างจากไฟล์จริงโดยอัตโนมัติ ห้ามแก้ตารางด้วยมือ; ให้รัน ' + tick + 'pipeline/docs.ps1 sync-index' + tick + ' หลังเพิ่ม ย้าย หรือเลิกใช้เอกสาร', '',
    '- เอกสารที่ควบคุมทั้งหมด: ' + documents.length + ' ฉบับ',
    '- Normative: ' + documents.filter(doc => doc.normative).length + ' ฉบับ', '',
  ];
  for (const [folder, docs] of [...byFolder.entries()].sort((a, b) => a[0].localeCompare(b[0], 'en'))) {
    lines.push('### ' + folder, '', '| เอกสาร | รหัส | สถานะ | ผู้รับผิดชอบ |', '|---|---|---|---|');
    for (const doc of docs) {
      const name = doc.path.split('/').slice(1).join('/') || doc.path;
      lines.push('| [' + escapeIndexCell(doc.title || name) + '](' + doc.path + ') | ' + tick + escapeIndexCell(doc.documentId) + tick
        + ' | ' + escapeIndexCell(doc.status) + ' | ' + escapeIndexCell(doc.owner) + ' |');
    }
    lines.push('');
  }
...
function syncDocumentIndex(inventory) {
  const file = path.join(ENTERPRISE_ROOT, 'DOCUMENT-INDEX.md');
  if (!fs.existsSync(file)) throw new Error('DOCUMENT-INDEX.md is missing.');
  const before = fs.readFileSync(file, 'utf8').replace(/^﻿/, '').replace(/\r\n?/g, '\n');
  const start = before.indexOf(INDEX_BLOCK_START);
  const end = before.indexOf(INDEX_BLOCK_END);
  if (start < 0 || end < 0 || end < start) throw new Error('DOCUMENT-INDEX.md has no valid GENERATED:DOCUMENT-INDEX block.');
  const after = before.slice(0, start) + renderDocumentIndexBlock(inventory) + before.slice(end + INDEX_BLOCK_END.length);
  if (after === before) {
    console.log('Document index is already current.');
    return false;
  }
  atomicWrite(file, after);
  console.log('Synchronized DOCUMENT-INDEX.md from ' + inventory.documents.length + ' controlled document(s).');
  return true;
}
...
function main() {
  try {
    ...
    const supported = new Set(['scan', 'impact', 'validate', 'preflight', 'status', 'accept', 'release-check', 'sync-index']);
    ...
    if (options.command === 'sync-index') {
      if (options.noWrite) console.log('No-write: DOCUMENT-INDEX.md was not modified.');
      else syncDocumentIndex(analysis.inventory);
      return;
    }
