...
> **หมายเหตุ:** เวอร์ชันปัจจุบัน = **1.6.7** · รายการ `[v5.x]` ด้านล่างเป็น**ประวัติ build ภายในก่อน reset** (คงไว้เพื่อ traceability ตาม ISO — ไม่ลบ)
>
...
พบระหว่างเดินเอกสารช่วงที่ 3 (ออกใบส่งของ 104 จากใบจอง I69-02432)

### Fixed

**แอปจ่ายเลขคีย์ทับบล็อกที่ WINSpeed จองให้เครื่องอื่น** — migration 089

WINSpeed **ไม่ได้ใช้ IDENTITY** กับ `SOHD.SOID` · `WFCoupon.CouponID` · `SMAudit.audit_id`
แต่แจกเลขเป็น **บล็อกละ 1000 ต่อเครื่อง** ผ่านตาราง `dbo.SMID` (`Name` = `<server>/<เครื่อง>/<ตาราง>`)
อ่านคำสั่งจริงได้จาก plan cache ตอนบันทึกใบ K69-01852
...
เห็นผลชัดตรงที่ SOID กระโดดจาก 274013 ไป **275000** ทั้งที่ `MAX(SOID)` ตอนนั้นคือ 274013
เพราะเครื่องที่ใช้งานเพิ่งได้บล็อก 275000

- เดิม `sp_ConfirmSalesOrder` ใช้ `MAX(SOID)+1` และ `winspeed-audit.js` ใช้ `MAX(audit_id)+1`
  เลขจึงไปนั่งในบล็อกของเครื่องอื่น — SOID 274001–274013 อยู่ในบล็อก 274000 ของ `WF-TEST-2`
  และ audit_id 1577001–1577026 อยู่ในบล็อก 1577000 ของอีกเครื่อง
  ยังไม่ชนเพราะทั้งสองฝั่งหาเลขว่างด้วย max-ภายในช่วง **แต่จะชนเมื่อบล็อกนั้นเต็ม**
- เพิ่ม `wf.usp_AllocateWinspeedId` ขึ้นทะเบียนแอปเป็นเครื่องหนึ่งใน SMID ชื่อ `wssale-app/backend/<ตาราง>`
  แล้วจ่ายเลขในบล็อกตัวเองด้วยกติกาเดียวกับ WINSpeed ทุกประการ
  (เลขแรกของบล็อกคือค่า `Prefix` เอง — ยืนยันจาก CouponID 242000 = บล็อก 242000)
- บล็อกที่ได้: `sohd` 276000 · `smaudit` 1578000 · `wfcoupon` 243000
- แถวเก่าที่เขียนไว้ในบล็อกของเครื่องอื่นปล่อยตามเดิม ไม่ย้อนแก้

⚠ เป็นการ INSERT ลง `dbo.SMID` ซึ่งเป็น dbo write นอกรายการที่อนุมัติไว้เดิม
เจ้าของระบบอนุมัติเป็นลายลักษณ์อักษรแล้ว 19/08/2569 · เพิ่มแถวอย่างเดียว ไม่แก้และไม่ลบของเดิม

### ค้างไว้ (แยกเป็นงานต่างหาก)

- แอปยังไม่มีด่านเตือน **เกินวงเงินเครดิต** ที่ WINSpeed เตือนตอนบันทึกใบส่งของ
  ("Sale Order Confirm Over Approve Credit AR") ควรเตือนตั้งแต่ตอนจอง
- คูปองที่เกิดพร้อมใบ 104 ยังได้ `CouponNo` ว่างและ `ContainQty = 0`
  (ของจริงคือ `D69xxxxx` และ 50 กก./กระสอบ) — เลขตั๋วออกที่ขั้นตอนถัดไป ไม่ใช่ที่ใบ 104

## [1.6.6] - 2026-08-19

ทดสอบ integration กับหน้าจอ WINSpeed จริงตั้งแต่ต้นจนอนุมัติผ่าน · แต่ละข้อพบจากการกดหน้าจอแล้วเจอ error ไม่ใช่จากการอ่านโค้ด

### Fixed

**ใบจากแอปไม่โผล่ในคิวอนุมัติเลย** — migration 086, 087

- อ่าน SQL ที่หน้าจอ Approve Confirm Order (WF) ยิงจริงจาก plan cache แทนการเดา ได้เงื่อนไขคิวเต็ม ๆ
  `DocuType=103 · onhold='N' · Clearso='N' · Docustatus NOT IN ('Y','P') · checkall='Y' · appvflag='W' · docudate ในเดือนปัจจุบัน`
- **`CheckAll` ต้องเป็น `'Y'`** — migration 084 ตั้งเป็น `'N'` เพราะผมอนุมานจากใบที่อนุมัติแล้วว่า `'Y'` คือผลของการอนุมัติ · ความจริงมันคือ **เงื่อนไขนำเข้าคิว** แปลว่า "ผู้เปิดใบตรวจรายการครบแล้ว พร้อมส่งให้ผู้ใหญ่อนุมัติ" (087)
- เติมฟิลด์ที่ WINSpeed ใส่ให้ทุกใบแต่แอปปล่อยว่าง (086) — สำรวจจากใบที่ WINSpeed สร้างเองปี 2569 จำนวน 4,346 ใบ
  `QuotStatus='รอผู้ใหญ่ตัดสินใจ'` (63,335/63,340 ของทั้งฐาน) · `VATGroupID=2` · `ValidDays`=เครดิตลูกค้า · `ShipDate`
  และช่องจำนวนเงิน `VATAmnt` `BaseDiscAmnt` `BillDiscAmnt` `SumIncludeAmnt` `MiscChargAmnt` `CommissionAmnt` `ResvAmnt1-4` = **0 ทั้ง 4,346 ใบ ไม่มีใบไหนเป็น NULL** · ปล่อย NULL ทำให้รายงานที่ SUM ข้ามคอลัมน์เหล่านี้เพี้ยนเงียบ ๆ
- `QuotStatus` ไม่ได้อยู่ในเงื่อนไขคิว แต่คงไว้เพราะทำให้ใบเหมือนของ WINSpeed จริงทุกฟิลด์

**WINSpeed ปฏิเสธด้วย "Salesman is not vaid!"** — migration 088

- `sp_ConfirmSalesOrder` มีบรรทัด `IF @EmpID IS NULL SET @EmpID = 1000` · ผู้ที่กดยืนยันซึ่งไม่มี `EmpId` ผูกไว้ (บทบาท `ADMIN` · `ACCOUNTING` · ผู้ดูแล) ทำให้ใบได้ `EmpID = 1000` เสมอ
- `EmpID 1000` อยู่ใน `dbo.EMEmp` และไม่ได้ลาออก **แต่ไม่ได้ขึ้นทะเบียนใน `dbo.EMSales`** ซึ่งเป็นทะเบียนพนักงานขายแยกต่างหาก (33 คน) ที่ WINSpeed ตรวจตอนอนุมัติ
- วัดจากใบ 103 ปี 2569 จำนวน 4,345 ใบ — อยู่ใน `EMSales` 4,289 · ว่าง 56 · **อยู่นอกทะเบียน 0 ใบ**
- ใบที่ผู้ดูแลกดยืนยันแทนพนักงานขายจึง **อนุมัติไม่ได้ทั้งใบ** และไม่มีสัญญาณจนกว่าจะมีคนไปกดอนุมัติ
- ตอนนี้ถ้า `EmpId` ไม่อยู่ใน `EMSales` ให้เป็น `NULL` แทนการเดา — WINSpeed ยอมรับค่าว่างและผู้อนุมัติเลือกเองบนหน้าจอได้

### Verification

ทดสอบวงจรเต็มบนฐาน UAT ด้วยหน้าจอ WINSpeed จริง

- `I69-02432` สร้างจากแอปในนามพนักงานขาย `emp-00035` (ชูชาติ · `EmpID 4000` ขึ้นทะเบียนใน `EMSales`)
- ผ่านด่านตรวจซ้ำ Counter-Sales (FR-022) แล้วยืนยัน → เข้าคิวอนุมัติทันที
- อนุมัติใน WINSpeed **ไม่มี warning เลย** → `AppvDocuNo = AI69-04544` · ผู้อนุมัติ `1023` · `EmpID` ไม่ถูกเขียนทับ
- ตรวจเงื่อนไขคิวทีละข้อ ผ่านครบ 6/6
- รอยเดินเอกสารต่อเนื่องข้ามระบบใน `dbo.SMAudit` — `990000001 wf:emp-00035` (แอปสร้าง) → `2098003080 USER-B` (WINSpeed อนุมัติ)

### Changed

- **แก้ความเข้าใจผิด** — เอกสารเคยระบุว่า "อนุมัติแล้วเกิดใบส่งของ 104 ทันที" · ทดสอบสองครั้งยืนยันว่า **ไม่จริง** · อนุมัติได้เลข `AI` อย่างเดียว · ใบ 104 · คูปอง · การลด `RemaQty` ไม่เกิด · ใบ 104 สร้างที่จอ **Sale Order (WF)** เป็นขั้นแยก (screen `2098003049`) ตรงตามผังของเจ้าของระบบ

## [1.6.5] - 2026-08-19

### Fixed

**ใบสั่งจองที่แอปสร้างติดค้างถาวรใน WINSpeed** — migration 084 · กลับทิศจาก migration 063

- `sp_ConfirmSalesOrder` เขียนใบด้วย `AppvFlag='Y'` · `DocuStatus='Y'` ทันทีตอนยืนยันในแอป โดยไม่ผ่านเมนู Approve Confirm Order (WF) · ได้สถานะลูกผสมที่ WINSpeed **ไม่เคยสร้างเอง** (`CheckAll` ว่าง · `AppvDocuNo` ว่าง)
- ทดสอบบนหน้าจอจริง — **Confirm Order (WF)** กด Delete และ F8 แล้วไม่มีอะไรเกิดขึ้น (ล็อกเพราะ `AppvFlag='Y'`) · **Approve Confirm Order (WF)** ค้นไม่เจอ (จอนี้แสดงเฉพาะ `AppvFlag='W'`) — ใบจึงแก้ไม่ได้ ลบไม่ได้ อนุมัติไม่ได้ และไม่มีวันได้เลข `AI`
- ตามมาด้วยไม่เกิดใบส่งของ 104 · ไม่เกิดคูปอง · ข้ามผู้มีอำนาจอนุมัติทั้งหมด
- 063 ตั้งใจทำเพื่อเลี่ยงการยืนยันสองรอบ — เหตุผลเดิมบันทึกไว้ในหัว 084 ครบ · เจ้าของระบบยืนยันแนวทางใหม่และรับทราบว่าต้องมีคนกดอนุมัติใน WINSpeed
- ตอนนี้สร้างเป็น `AppvFlag='W'` · `DocuStatus='N'` · `CheckAll='N'` **ตรงกับใบที่คีย์ใน WINSpeed ทุกธง** (เทียบกับ `I69-02420` แล้ว)
- ปลดใบที่ค้างจากรุ่นเดิม 3 ใบกลับเป็นรออนุมัติ พร้อมบันทึกเหตุผลลง `dbo.SMAudit`

### Added

**สถานะ `PENDING_APPROVAL` — "รออนุมัติใน WINSpeed"** — migration 085

- `wf.v_AllSalesOrders` ไม่เคยดู `AppvFlag` เลย ใบที่รออนุมัติจึงแสดงเป็น "รอจัดส่ง" · คลังอาจเริ่มจัดของให้ใบที่ผู้มีอำนาจยังไม่เห็น
- เงื่อนไขวางไว้**ท้ายสุด**ถัดจาก `IsUnlocked` เท่านั้น — ใบที่ชั่งออกหรือออกใบกำกับแล้วต้องไม่ถอยกลับมาเป็นรออนุมัติ แม้ธงจะค้างเป็น `'W'`
- ตรวจแล้ว: `I69-02420` `02421` `02422` ที่ `AppvFlag='W'` แต่ส่งของแล้ว **ยังคงเป็น SHIPPED** ส่วนใบใหม่จากแอปเป็น `PENDING_APPROVAL` ถูกต้อง

## [1.6.4] - 2026-08-19

### Fixed

**เลขที่ใบสั่งขายข้ามแบบทวีคูณ** — `backend/routes/so.js`

- `allocateWorkflowRef()` ออกเลขด้วย `MAX + NEXT VALUE FOR wf.WfRefSeq` · `WfRefSeq` เป็นตัวนับที่โตขึ้นเรื่อยและไม่เคยรีเซ็ต **เลขที่เพิ่งจองกลายเป็น MAX ของรอบถัดไป แล้วถูกบวกด้วยค่าที่โตขึ้นอีก** ช่องว่างจึงขยายแบบกำลังสอง
- วัดจริงบน UAT เริ่มจาก `I69-02422` — สร้างสามใบติดกันได้ `I69-02425` · `I69-02428` · `I69-02432` คือ **กินเลข 10 หมายเลข ข้ามทิ้ง 7**
- เลขที่เอกสารขายเป็นหลักฐานทางภาษี ช่องว่างต้องอธิบายได้เสมอ
- เปลี่ยนเป็น `MAX + 1` พร้อม `UPDLOCK, HOLDLOCK` บน `wf.SalesOrder` (อ่าน `dbo.SOHD` ด้วย `NOLOCK` เท่านั้น — ห้ามล็อกตารางของ WINSpeed) · ความปลอดภัยจากการชนมาจาก unique index บน `WfRef` ที่มีอยู่แล้ว
- ยืนยันแล้วว่านับรวมเอกสารที่คีย์ตรงใน WINSpeed ด้วย — เริ่มที่ `I69-02424` เพราะ `I69-02423` ถูกคีย์ใน WINSpeed ไปแล้ว

### Added

**เดินตัวนับเลขที่เอกสารของ WINSpeed** — `backend/services/winspeed-counter.js`

- แอปออกเลขเองแล้วเขียนลง `dbo.SOHD` แต่ไม่เคยบอก `dbo.EMRunBrch` ซึ่งเป็นตัวนับที่หน้าจอ WINSpeed ใช้เสนอเลขถัดไป · ยืนยันสองใบแล้วตัวนับยังค้างที่เลขเดิม **พนักงานที่เปิดใบใหม่ใน WINSpeed จะได้เลขที่ถูกใช้ไปแล้ว** และไล่ชนต่อไปทุกใบ
- เป็นกลไกเดียวกับที่ทำให้ `J69-02806` ชนกันเมื่อ 18 ส.ค. ต่างกันแค่คราวนั้นเกิดจากสคริปต์ คราวนี้เกิดจากตัวแอปเอง
- ⚠ เป็นการ **แก้ค่าเดิมใน `dbo`** จึงจำกัดขอบเขตแคบที่สุด — แตะเฉพาะ `LastNo` ของแถวที่คุมชุดเลขนั้น · **ไม่เคยถอยหลัง** (อัปเดตเมื่อเลขใหม่มากกว่าเดิมเท่านั้น ถ้าพนักงานแซงไปแล้วค่าของเขาชนะ) · เลขที่ผิดรูปแบบไม่แตะตัวนับ · ล้มเหลวไม่ทำให้การยืนยันล้มตาม
- ทดสอบครบสามเงื่อนไข: เดินขึ้นสำเร็จ · ปฏิเสธเลขที่ต่ำกว่า · ปฏิเสธรูปแบบที่ไม่เข้าเกณฑ์

## [1.6.3] - 2026-08-19

### Added

**รอยการทำงานของแอปปรากฏใน `dbo.SMAudit` ของ WINSpeed** — `backend/services/winspeed-audit.js`

- WINSpeed บันทึกทุกการกระทำไว้ที่ `dbo.SMAudit` (586,753 แถว ปี 2564–2569) · เอกสารที่แอปสร้างหรือแก้จึงเคยปรากฏใน WINSpeed **โดยไม่มีรอยว่าใครทำ เมื่อไร ด้วยเครื่องอะไร**
- เขียนเฉพาะจังหวะที่แอปเปลี่ยน `dbo` จริง (ยืนยันใบสั่งขาย · จัดสินค้า · ปลดล็อก) และการชั่งออก · **ไม่ได้ย้าย** `wf.SalesOrderAudit` มาที่นี่ — ตัวนั้นยังเป็นบันทึกหลักเพราะเก็บการเปลี่ยนสถานะได้ละเอียดกว่า
- จองช่วง `audit_screen` = `990000001`–`990000004` ซึ่ง **ตรวจแล้วว่า WINSpeed ไม่ได้ใช้ทั้งช่วง 900000000–999999999** · `audit_username` ขึ้นต้น `wf:` เสมอ · กรองแยกออกจากรายงานเดิมได้ทันที
- `audit_id` **ไม่ใช่ IDENTITY** (เหมือน `SOID`/`SOInvID`) จึงคำนวณ MAX+1 ในคำสั่งเดียวกับ INSERT และลองใหม่เมื่อชนคีย์
- การเขียนรอยล้มเหลวจะไม่ทำให้การชั่งออกหรือยืนยันคำสั่งขายล้มตาม แต่ขึ้น log เสมอ
- ⚠ นี่คือการ **INSERT ลง `dbo`** ซึ่งเป็นข้อยกเว้นที่เจ้าของระบบอนุมัติเฉพาะเรื่องนี้ · เพิ่มแถวอย่างเดียว ไม่แก้และไม่ลบข้อมูลเดิม

ตรวจกับฐาน UAT แล้ว — `I69-02420` มีรอยต่อเนื่อง: WINSpeed สร้างเมื่อ 12 ส.ค. (หน้าจอ 2098003048 ผู้ใช้ `USER-B`) → แอปจัดสินค้าและชั่งออกเมื่อ 19 ส.ค. (หน้าจอ 990000002/990000004 ผู้ใช้ `wf:admin`)

### Fixed

**เที่ยวรถถูกแตกเป็นหลายกลุ่มทันทีที่ส่งลูกค้ามากกว่าหนึ่งราย** — `WSSale-App/src/components/sales/`

- จัดกลุ่มด้วย วันที่ + **ลูกค้า** + ทะเบียน + สถานะ · รถที่ขึ้นของเที่ยวเดียวแล้วแวะส่งหลายรายตามเส้นทางจึงแสดงเป็นหลายเที่ยว คนจัดลำดับขึ้นของมองไม่เห็นว่ารถคันนั้นต้องบรรทุกอะไรบ้าง — ซึ่งเป็นเหตุผลทั้งหมดที่ฟีเจอร์นี้มี เพราะ WINSpeed ทำ Sale Trip ไม่ได้
- **เป็นกรณีปกติ ไม่ใช่ข้อยกเว้น** — วัดจากใบสั่งขายจริงตั้งแต่ ม.ค. 2568: เที่ยวรถ 9,849 · มีมากกว่าหนึ่งใบ 1,297 · ในนั้นส่งหลายลูกค้า **1,203 (93%)**
- เปลี่ยนเป็น วันที่ + ทะเบียน + สถานะ · การ์ดเที่ยวรถและหน้าสรุปแสดงจำนวนลูกค้าและรายชื่อครบทุกราย แทนที่จะโชว์รายแรกรายเดียว

## [1.6.2] - 2026-08-15

### Fixed

**ใบสั่งขายที่คีย์ใน WINSpeed เดินไม่ถึงขั้นชั่งออก** — `backend/routes/so.js`

- ขั้นโหลดสินค้าสั่ง `UPDATE wf.SalesOrderExt SET IsLoaded=1` แต่ใบที่พนักงานคีย์ใน WINSpeed โดยตรงไม่มีแถวนั้น (แถวนี้สร้างโดย `wf.sp_ConfirmSalesOrder` ซึ่งทำงานเฉพาะใบที่เปิดจากแอป) คำสั่งจึงโดน **0 แถวโดยไม่มี error** สถานะค้างที่ `PICKING` และขั้นชั่งออกซึ่งบังคับ `LOADED` ตอบ 400 ทุกครั้ง
- ผลกระทบ: **ใบส่วนใหญ่ของระบบจริงเดินไม่จบสาย** — ไม่ปรากฏใน UAT เพราะทดสอบด้วยใบที่เปิดจากแอปเท่านั้น
- แก้ด้วย `ensureSalesOrderExt()` ที่สร้างแถวให้เมื่อยังไม่มี เรียกจากขั้นจัดสินค้า/โหลด/ชั่งออก · ทำงานซ้ำได้ · **อ่าน `dbo` อย่างเดียว** ไม่เขียนกลับ
- `SalesUserId` เทียบจาก `dbo.SOHD.EmpID` → `wf.AppUser.EmpId` ทำให้รีเบทเข้าของพนักงานขายเจ้าของใบ **ไม่ใช่เจ้าหน้าที่เครื่องชั่งที่กดปุ่ม** (เดิมตกไปใช้ `req.user.sub` เพราะ `so` ถูกอ่านมาก่อนแถวจะเกิด)
- ตรวจแล้วบนฐาน UAT: `I69-02421` เดิน `CONFIRMED → PICKING → LOADED → SHIPPED` ครบผ่าน HTTP · เขียนกลับ MySQL `tblscale` สำเร็จ (s_id 430571) · `wf.WeighTicket` บันทึกผลประเมินน้ำหนักครบ

**`DB_MODE=remote_b` ลงฐานเครื่องพัฒนาเงียบ ๆ** — `backend/db.js`

- `db.js` แปลง DB_MODE ด้วย `=== 'remote' ? 'remote' : 'local'` จึงตีความ **ทุกค่าที่ไม่ใช่ `remote` เป็น `local`** โดยไม่เตือน · `DB_MODE=remote_b` ที่ทีมใช้กันมาตลอดจึงต่อฐานผิดเครื่องทุกครั้ง
- อันตรายเพราะสคริปต์ที่เขียนข้อมูล เช่น `audit-duplicate-passwords.js --fix` จะไปแก้ฐานผิดโดยไม่มีสัญญาณใด ๆ · `scripts/migrate-targets.js` ไม่โดนเพราะแมป `REMOTE_B_*` ทับ `REMOTE_*` แล้วเรียก child process เอง
- เพิ่ม `remoteBConfig()` แยกต่างหาก และ **ค่า DB_MODE ที่ไม่รู้จักให้ throw ทันที** แทนที่จะตกไป local
- ตรวจแล้วทั้งสามปลายทางต่อไปยังเซิร์ฟเวอร์คนละเครื่องจริง — ยืนยันด้วย `SELECT @@SERVERNAME` ของแต่ละปลายทาง

**ผู้ใช้ C_LEVEL คนเดียวเซ็นครบทั้งสี่ชั้นได้เอง** — `backend/routes/rebate.js`

- กฎแบ่งแยกหน้าที่ยกเว้น `['ADMIN','C_LEVEL']` ไว้ทั้งกลุ่ม ทำให้ผู้ถือบทบาทนั้นอนุมัติชั้น 2 → 3 → 4 ได้ด้วยตัวคนเดียว · ตอนนั้น `C_LEVEL` มี 20 บัญชี ลายเซ็นสี่ชั้นจึงไม่มีความหมายเลย
- เหลือทางยกเว้นเดียวคือ `ALLOW_SINGLE_USER_MULTI_TIER_APPROVAL` ซึ่งตั้งได้เฉพาะผู้ดูแลเซิร์ฟเวอร์

**`seed_admin.js` เขียนทับบทบาทและรหัสผ่านทุกครั้งที่รัน** — `backend/seed_admin.js`

- ทุกการรันสั่ง `UPDATE ... SET PasswordHash = @h, Role = @r` ทับบัญชีที่มีอยู่ · **นี่คือที่มาของบัญชีรหัสผ่านซ้ำ 41 รายการ** ที่ `audit-duplicate-passwords` ตรวจพบ และทำให้บทบาทที่จัดไว้ตาม migration 082 ถูกย้อนกลับ
- ลำดับใน `bootstrap.sh` คือ migrate → seed_admin · ถ้า seed เขียนทับ **เครื่องที่ติดตั้งใหม่จะได้บทบาทคนละชุดกับเครื่องที่ใช้งานอยู่**
- ตอนนี้บัญชีที่มีอยู่แล้วจะถูกอัปเดตเฉพาะ `Username` / `DisplayName` / `EmpId` — **ไม่แตะบทบาทและรหัสผ่าน**
- เกณฑ์บทบาทของบัญชีใหม่เดิมใช้ `EmpGroupID = '2000'` → `C_LEVEL` · กลุ่ม 2000 คือ **"สำนักงานใหญ่" ซึ่งเป็นสถานที่ทำงาน ไม่ใช่ระดับตำแหน่ง** จึงแจกสิทธิ์สูงสุดให้พนักงานบัญชีและธุรการ 22 คน · เปลี่ยนมาอ่าน `dbo.EMPost` / `dbo.EMDept` / `dbo.EMEmpGroup` ให้ตรงกับเกณฑ์ของ migration 082
- ทดสอบเทียบกับฐานจริงก่อนแก้: เกณฑ์ใหม่ให้ผลตรงกับบทบาทปัจจุบันทุกบัญชี ยกเว้นหนึ่งรายที่อยู่กลุ่มคลังสินค้า — ปิดช่องว่างนั้นด้วย migration 083

### Changed

**บทบาทผู้ใช้ตรงกับตำแหน่งงานจริง** — migration 082 (DECISIONS-v1.6.0 ข้อ 2 · ทางเลือก 2ก)

- `C_LEVEL` ลดจาก 23 บัญชีเหลือ **กรรมการบริหารสองคน** · ผู้จัดการภาคสามคนเป็น `MANAGER` · ฝ่ายบัญชีสองคนเป็น `ACCOUNTING` · ที่เหลือเป็น `SALES`
- เกณฑ์ตัดสินอ่านจาก `dbo.EMPost` / `dbo.EMDept` และจำนวนเอกสารที่ออกจริง ไม่ใช่การเดา · บัญชีที่ไม่มีหลักฐานถูก**ลดสิทธิ์เสมอ ไม่เคยเพิ่ม**
- คืน `wf.UserSaleArea` ทั้ง 7 ภาคที่หายไปตอน restore ฐาน — ถ้าตารางนี้ว่าง ชั้นที่ 2 จะเหลือแต่การตรวจด้วยบทบาทอย่างเดียว
- ชั้นที่ 3 รับเฉพาะ `C_LEVEL`/`ADMIN` แล้ว · เดิมรับ `MARKETING`/`MANAGER`/`APPROVER` ซึ่งเป็นชุดคนเดียวกับชั้น 2
- ไม่ใช้บทบาท `MARKETING` เพราะบริษัทไม่มีฝ่ายการตลาด — ตรวจ `dbo.EMPost` ครบทั้ง 10 ตำแหน่งแล้วไม่มี
- migration 083 ให้พนักงานกลุ่ม `คลังสินค้า` เป็น `WAREHOUSE` — เพื่อให้เครื่องที่ใช้งานอยู่ตรงกับเครื่องที่ติดตั้งใหม่ทุกประการ

### Verification

- `verify-rebate-4tier` — ผ่านทุกข้อ · **ลายเซ็นครบ 4 ชั้นจากผู้อนุมัติ 4 คนที่ต่างกันจริง**
- `verify-rbac-matrix` (24 กรณี) · `verify-rebate-plan-approval` · `verify-rebate-single-source` · `verify-password-gate` — ผ่านทั้งหมด

### Changed

- [WINSPEED-DOCUMENT-FLOW](WINSPEED-DOCUMENT-FLOW.md) เป็น v1.1 — ปิดข้อค้าง 3 ข้อจากการทดลองกับเอกสารจริง: การยกเลิกเอกสาร · ชื่อเมนู WINSpeed · ความต่างของ `SOInvHD` 202 กับ 107
...
- **พบว่าปลายทางของกระดาษคือ `dbo.SOInvHD` Docutype 106** เลขที่ `RB<รหัสผู้ขอ><ปี พ.ศ.>-<ลำดับ>` · 16,195 ใบ · ยืนยันกับใบกระดาษ `RBD68-049` ตรงทุกช่อง (วันที่ 7/05/2568 · CustID 23037 · 55,800 บาท)
...
- **ที่ผูกไว้เดิมผิด** — `emp-00036` ถูกผูกกับภาคใต้ ทั้งที่ยอดขาย 16,848 ใบอยู่ภาคอีสาน และไม่มีทางแก้จากระบบเลย · แต่งตั้งใหม่ครบทั้ง 7 ภาคจากสายบังคับบัญชาจริงใน `dbo.EMEmp.EmpHead` ไม่ใช่จากผู้ที่ทำยอดสูงสุด เพราะผู้ทำยอดสูงสุดเป็นเพื่อนร่วมงานระดับเดียวกับผู้ยื่นเรื่อง
- **บริษัทไม่มีฝ่ายการตลาด** — `dbo.EMPost` มี 10 ตำแหน่งและไม่มีตำแหน่งใดเกี่ยวกับการตลาด · ใช้กรรมการบริหารสองคนที่มีจริงเป็นชั้น 3 และ 4 แทน (`emp-00016` และ `emp-00059` เลื่อนเป็น `C_LEVEL`)
