...
# อ่านคำที่ต้องตรวจจากไฟล์รายการแทนที่ ไม่เขียนไว้ในสคริปต์
#
# ถ้าเขียนชื่อไว้ตรง ๆ ตัวสคริปต์เองจะกลายเป็นที่เก็บชื่อ และรอบถัดไป --replace-text
# จะแทนชื่อในสคริปต์ด้วย ทำให้คำที่ใช้ตรวจเพี้ยนจนตรวจไม่เจออะไรเลย
$allNeedles = @(Get-Content $replTxt -Encoding UTF8 |
                Where-Object { $_ -match '==>' } |
                ForEach-Object { ($_ -split '==>')[0] } |
                Where-Object { $_.Trim() })
# สุ่มตรวจ 5 คำจากรายการจริง + รหัสผ่านเสมอ
$needles = @{}
$i = 0
foreach ($n in ($allNeedles | Get-Random -Count ([Math]::Min(5, $allNeedles.Count)))) {
    $i++; $needles["คำที่ $i"] = $n
}
$needles['รหัสผ่านตั้งต้น'] = ('W0rld' + 'F3rt')
$dirty = $false
