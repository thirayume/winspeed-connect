...
export function SODetailsPanel({
    ...
    if (isInline) {
      return (
        <div className="flex flex-col items-center justify-center h-full text-center p-8 opacity-40">
           ...
           <p className="text-xs mt-1 text-muted-foreground">Select an order from the list to view details.</p>
        </div>
      );
    }
    return null;
  ...
  const content = (
    <div className={cn(
      ...
      !isInline && "w-full max-w-md shadow-2xl sm:border-l border-border"
    )}>
      ...
      <div className="border-t border-border bg-slate-50/50 p-6 space-y-3 pb-12 sm:pb-6 mt-auto">
        {so.Status === 'Draft' && (
          <Button className="w-full h-12 text-base font-bold rounded-xl shadow-sm" onClick={() => setShowConfirmAction(true)} disabled={busy}>
            Confirm Order
          </Button>
        )}
        {(so.Status === 'Confirmed' || so.Status === 'Picking') &&
          (pendingReq ? (
            ...
            <Button
              variant="primary"
              className="w-full h-12 bg-[var(--color-status-locked)] hover:bg-[var(--color-status-locked)]/90 font-bold rounded-xl shadow-sm"
        ...
        <div className="grid grid-cols-2 gap-3">
          <Button variant="outline" className="h-11 rounded-xl" disabled={locked || busy} onClick={handleEdit}>
            Edit Order
          </Button>
          <Button variant="outline" className="h-11 text-red-600 hover:text-red-700 hover:bg-red-50 rounded-xl" disabled={locked || busy} onClick={() => setShowCancelAction(true)}>
            Cancel SO
          </Button>
        </div>
      </div>
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed inset-y-0 right-0 z-50 w-full max-w-md overflow-hidden bg-white shadow-2xl flex flex-col"
          >
            {content}
          </motion.div>
        </>
