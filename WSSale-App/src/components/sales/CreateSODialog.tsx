...
import { createSO, fetchCustomers, fetchItems } from "../../services/api";
import type { EMCust, EMGood, SOLine } from "../../types";

export function CreateSODialog({ isOpen, onClose, onCreated }: { isOpen: boolean, onClose: () => void, onCreated?: () => void }) {
  const [customers, setCustomers] = useState<EMCust[]>([]);
  const [items, setItems] = useState<EMGood[]>([]);
  const [custId, setCustId] = useState("");
  const [requestedDate, setRequestedDate] = useState(new Date(Date.now() + 7 * 86400000).toISOString().slice(0, 10));
  ...
  const reset = () => {
    setCustId("");
    setRequestedDate(new Date(Date.now() + 7 * 86400000).toISOString().slice(0, 10));
    setLines([]);
  };
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await createSO({ CustID: custId, RequestedDate: requestedDate, lines: lines } as any);
      onCreated?.();
      onClose();
      reset();
    } catch (err) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="New Sales Order" size="full">
