...
import { Coins, RefreshCw, ChevronRight, ChevronLeft, Users, Package, X } from 'lucide-react';
import { fetchRebateSummary, fetchCouponCustomers, fetchCouponDetail, fetchRebateClaims, approveRebateClaim } from '../../services/api';
import { useAuthStore } from '../../store/auth-store';
import type { RebateSummary, CouponCustomer, CouponRow, RebateClaim } from '../../types';
...
export function RebatePage() {
  ...
  const [selEmp, setSelEmp]         = useState<RebateSummary | null>(null);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [s, c] = await Promise.all([fetchRebateSummary(), fetchRebateClaims()]);
      setSummary(s);
      setClaims(c);
  ...
  async function drillEmp(emp: RebateSummary) {
    setSelEmp(emp);
    setLoading(true);
    try {
      const r = await fetchCouponCustomers({ empId: Number(emp.EmpID) });
      setCustomers(r);
      setView('customer');
    } catch (e) { console.error(e); }
    setLoading(false);
  ...
  async function drillCust(cust: CouponCustomer) {
    ...
    setLoading(true);
    try {
      const r = await fetchCouponDetail(cust.CustID);
      setCoupons(r);
      setView('coupon');
    } catch (e) { console.error(e); }
    setLoading(false);
  ...
  async function doApprove(claim: RebateClaim) {
    const cn = prompt(`กรอกเลข CN (Credit Note 109) สำหรับเคลม ฿${Number(claim.ClaimAmt).toLocaleString()}:\n(เว้นว่างได้)`);
    if (cn === null) return;
    setBusyId(claim.Id);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black flex items-center gap-2" style={{ color: '#0C447C' }}>
              <Coins size={26} />
              {view === 'summary' && 'รีเบท (Coupon คงค้าง)'}
              {view === 'customer' && `ลูกค้าของ ${selEmp?.EmpName}`}
              {view === 'coupon'   && `คูปองของ ${selCust?.CustName}`}
            </h1>
            <p className="text-sm text-gray-500 mt-0.5">
              {view === 'summary' && 'ยอดคูปองคงค้างจาก Winspeed · แยกตามพนักงานขาย'}
              {view === 'customer' && `${customers.length} ลูกค้า · คลิกเพื่อดูคูปองรายการ`}
              {view === 'coupon'   && `${coupons.length} ใบ คงค้าง`}
            </p>
          </div>
        </div>
          ...
          <>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex items-center gap-3">
                ...
                <div>
                  <div className="text-xs text-gray-400">คงค้างรวม</div>
                  <div className="text-2xl font-bold text-gray-800">{totalTon.toLocaleString(undefined,{maximumFractionDigits:1})} ตัน</div>
                </div>
              </div>
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex items-center gap-3">
                ...
                <div>
                  <div className="text-xs text-gray-400">ลูกค้ามี coupon</div>
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex items-center gap-3">
                ...
                <div>
                  <div className="text-xs text-gray-400">เคลมรออนุมัติ</div>
                  <div className="text-2xl font-bold text-gray-800">{pendingClaims.length}</div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-100">
                <h2 className="text-sm font-bold text-gray-700">ยอดคงค้างแยกตามพนักงานขาย</h2>
              ...
              {loading ? (
                ...
                <p className="text-xs text-gray-400 py-8 text-center">ไม่มีคูปองคงค้าง</p>
              ) : (
                      ...
                      <button key={emp.EmpID} onClick={() => drillEmp(emp)}
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between mb-1">
                            ...
                            <span className="text-sm font-bold text-[#0C447C]">{Number(emp.OutstandingTon).toLocaleString(undefined,{maximumFractionDigits:1})} ตัน</span>
                          </div>
                          <div className="flex items-center gap-3">
                            <div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                              <div className="h-full rounded-full bg-[#0C447C]" style={{ width: `${pct}%` }} />
                            </div>
                            <span className="text-xs text-gray-400 shrink-0">{emp.CustCount} ลูกค้า · {emp.CouponCount} ใบ</span>
                          </div>
                        </div>
            ...
            {/* Pending claims */}
            {pendingClaims.length > 0 && (role === 'ACCOUNTING' || role === 'ADMIN' || role === 'MANAGER') && (
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                <h2 className="text-sm font-bold text-gray-700 mb-3 flex items-center gap-2">
                  <Coins size={16} className="text-amber-500" /> เคลมรออนุมัติ
                </h2>
        ...
        {view === 'customer' && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100">
              ...
              <p className="text-xs text-gray-400 mt-0.5">คลิกเพื่อดูรายการคูปอง</p>
            </div>
            {loading ? (
              <div className="py-12 flex justify-center"><RefreshCw size={24} className="animate-spin text-gray-300" /></div>
            ) : customers.length === 0 ? (
              <p className="text-xs text-gray-400 py-8 text-center">ไม่มีลูกค้า</p>
            ) : (
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 text-xs text-gray-500 uppercase">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-center">ใบคูปอง</th>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {customers.map(c => (
                      <tr key={c.CustID} onClick={() => drillCust(c)}
        ...
        {view === 'coupon' && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100">
              ...
              <p className="text-xs text-gray-400 mt-0.5">Sales: {selCust?.EmpName} · คูปองที่ยังไม่เบิก</p>
            </div>
            {loading ? (
              <div className="py-12 flex justify-center"><RefreshCw size={24} className="animate-spin text-gray-300" /></div>
            ) : coupons.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 text-xs text-gray-500 uppercase">
                    <tr>
                      <th className="px-4 py-3 text-left">เลขคูปอง</th>
