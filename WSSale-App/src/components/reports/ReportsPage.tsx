  ...
  return (
    <div className="h-full flex flex-col" style={{ background: '#F1EFE8' }}>
      <div className="px-6 py-5 border-b border-gray-200 bg-white shadow-sm flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black flex items-center gap-2" style={{ color: '#0C447C' }}><BarChart3 size={26} /> รายงาน (Reports)</h1>
          <p className="text-sm text-gray-500 mt-0.5">สรุปข้อมูล + ส่งออก Excel (FR-017)</p>
        </div>
      ...
      <div className="flex-1 overflow-hidden flex">
        {/* report list */}
        <div className="w-64 border-r border-gray-200 bg-white/60 overflow-y-auto p-3 space-y-1 shrink-0">
          {types.map(t => (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-50">
                    ...
                    {data.rows.length === 0 && <tr><td colSpan={data.columns.length} className="py-10 text-center text-gray-300">ไม่มีข้อมูล</td></tr>}
                  </tbody>
                </table>
              </div>
            </div>
