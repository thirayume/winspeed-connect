        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black flex items-center gap-2" style={{ color: '#0C447C' }}>
              <Ticket size={26} /> {sel ? `ตั๋วคุม ${sel.DocuNo}` : 'ชุดตั๋วคุม (Control Ticket)'}
            </h1>
            <p className="text-sm text-gray-500 mt-0.5">{sel ? `${sel.CustName} · คงเหลือ ${rem(sel).toFixed(2)} ตัน` : 'ยอดคงค้าง (Total − Drawn) · เจาะดูรายการ+ประวัติการตัด (FR-021)'}</p>
          </div>
        </div>
        ...
        {!sel ? (
            ...
            {loading ? (
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50 text-xs text-gray-500 uppercase">
                  <tr>
                    <th className="px-4 py-3 text-left">เลขตั๋ว (AI)</th>
                    <th className="px-4 py-3 text-left">ลูกค้า</th>
                    <th className="px-4 py-3 text-center">วันที่</th>
                    <th className="px-4 py-3 text-right">จอง (ตัน)</th>
                    <th className="px-4 py-3 text-right">ตัดแล้ว</th>
                    <th className="px-4 py-3 text-left w-48">คงเหลือ</th>
                  ...
                  {filtered.map(t => {
                    ...
                    return (
                      <tr key={String(t.SOID)} onClick={() => open(t)} className="hover:bg-blue-50/40 cursor-pointer">
                        <td className="px-4 py-2.5 font-mono font-bold text-[#0C447C]">{t.DocuNo}</td>
                        <td className="px-4 py-2.5 text-gray-700 max-w-[180px] truncate" title={t.CustName}>{t.CustName}</td>
                        <td className="px-4 py-2.5 text-center text-xs text-gray-400">{t.DocuDate?.substring(0, 10)}</td>
                        <td className="px-4 py-2.5 text-right text-gray-600">{total.toFixed(2)}</td>
                        <td className="px-4 py-2.5 text-right text-gray-400">{drawn.toFixed(2)}</td>
          ...
          <div className="space-y-5">
            ...
            <Panel title="รายการในชุดตั๋ว" loading={detailLoading}>
              <table className="w-full text-sm">
                <thead className="bg-gray-50 text-xs text-gray-500 uppercase"><tr>
                  <th className="px-4 py-2 text-left">สูตร</th><th className="px-4 py-2 text-right">ตัน</th><th className="px-4 py-2 text-right">ราคา/ตัน</th>
                </tr></thead>
                <tbody className="divide-y divide-gray-50">
                  {lines.map(l => <tr key={l.ListNo}><td className="px-4 py-2">{l.GoodName || l.GoodCode}</td><td className="px-4 py-2 text-right">{Number(l.QtyTon).toFixed(2)}</td><td className="px-4 py-2 text-right text-gray-500">{Number(l.PricePerTon).toLocaleString()}</td></tr>)}
                  {lines.length === 0 && <tr><td colSpan={3} className="py-6 text-center text-gray-300">—</td></tr>}
                </tbody>
              </table>
            </Panel>
            ...
            <Panel title={`ประวัติการตัด (${draws.length})`} loading={detailLoading}>
              <table className="w-full text-sm">
                <thead className="bg-gray-50 text-xs text-gray-500 uppercase"><tr>
                  <th className="px-4 py-2 text-left">เลข SO (104)</th><th className="px-4 py-2 text-center">วันที่</th><th className="px-4 py-2 text-left">รถ</th><th className="px-4 py-2 text-right">ตัดออก (ตัน)</th>
                </tr></thead>
                <tbody className="divide-y divide-gray-50">
                  {draws.map(d => <tr key={String(d.SOID)} className="hover:bg-gray-50/50">
                    <td className="px-4 py-2 font-mono text-xs text-[#0C447C]">{d.DocuNo}</td>
                    <td className="px-4 py-2 text-center text-xs text-gray-400">{d.DocuDate}</td>
                    <td className="px-4 py-2 text-xs text-gray-500 flex items-center gap-1">{d.TruckPlate ? <><Truck size={11} />{d.TruckPlate}</> : '-'}</td>
                    <td className="px-4 py-2 text-right font-semibold"><span className="inline-flex items-center gap-1"><ArrowDownToLine size={12} className="text-amber-500" />{Number(d.DrawnQtyTon).toFixed(2)}</span></td>
                  </tr>)}
                  {draws.length === 0 && <tr><td colSpan={4} className="py-6 text-center text-gray-300">ยังไม่มีการตัด</td></tr>}
                </tbody>
              </table>
            </Panel>
          </div>
...
function Stat({ label, value, color }: { label: string; value: string; color: string }) {
  return <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4"><div className="text-xs text-gray-400">{label}</div><div className="text-2xl font-bold" style={{ color }}>{value}</div></div>;
}
