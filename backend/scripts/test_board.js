...
async function run() {
  try {
      ...
      WITH Orders AS (
        ...
        WHERE w.DedupRN = 1
          AND (w.Status IN ('DRAFT', 'CONFIRMED', 'PICKING', 'LOADED')
               OR (w.Status IN ('SHIPPED', 'IMPORTED') AND w.CreatedAt >= DATEADD(day, -30, GETDATE())))
      ),
      RankedSO AS (
    ...
    const target = res.filter(r => r.ImportedDocuNo === 'I69-02473' || r.ControlTicketNo === 'I69-02473' || r.ImportedDocuNo === 'I69-02484' || r.ImportedDocuNo === 'I69-02496');
    console.log(target);
  } catch (e) {
    console.error(e);
  } finally {
