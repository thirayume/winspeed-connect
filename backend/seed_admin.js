...
const USERS = [
  { username: 'admin',       password: '***REMOVED-PASSWORD***',     displayName: 'ผู้ดูแลระบบ',            role: 'ADMIN',      empId: null }
];
...
async function seed() {
  ...
  const defaultPasswordHash = await bcrypt.hash('***REMOVED-PASSWORD***', 12);

  for (const u of USERS) {
    const hash = u.password === '***REMOVED-PASSWORD***' ? defaultPasswordHash : await bcrypt.hash(u.password, 12);
