-- =============================================================
-- 086_confirm_so_winspeed_required_fields.sql
--
-- ใบจากแอปไม่โผล่ในคิวอนุมัติ เพราะขาดฟิลด์ที่ WINSpeed ใส่ให้ทุกใบ
--
-- อาการ
--   migration 084 ตั้งธงถูกแล้ว (AppvFlag='W' · DocuStatus='N' · CheckAll='N')
--   แต่หน้าจอ Approve Confirm Order (WF) → New → Ref Confirm Order
--   ยังค้นไม่เจอใบจากแอปเลยแม้แต่ใบเดียว
--
-- ต้นเหตุ — QuotStatus
--   นับจากใบสั่งจอง 103 ทั้งฐาน 63,340 ใบ
--     QuotStatus = N'รอผู้ใหญ่ตัดสินใจ'   63,335 ใบ  (99.992%)
--     QuotStatus = NULL                        5 ใบ  ← ของแอปเราล้วน
--   ไม่มีใบไหนในประวัติเจ็ดปีที่ WINSpeed สร้างแล้วปล่อยฟิลด์นี้ว่าง
--   หน้าจออนุมัติกรองด้วยฟิลด์นี้ ใบเราจึงไม่เข้าคิว
--
-- ฟิลด์อื่นที่ขาดด้วย — สำรวจจากใบที่ WINSpeed สร้างเองปี 2569 จำนวน 4,346 ใบ
--   VATGroupID  = 2        ทุกใบ (คู่กับ VatType='3' VatRate=0 ที่ SP ใส่อยู่แล้ว)
--   ValidDays   = CreditDays  ตรงกัน 4,036 จาก 4,346 ใบ (93%)
--   ShipDate    ห่างจาก DocuDate เท่ากับ ValidDays พอดี — WINSpeed คำนวณให้เอง
--               แอปมีวันนัดส่งจริงจากผู้ใช้ จึงใช้ค่านั้นก่อน แล้วค่อยตกมาที่สูตรของ WINSpeed
--   SumIncludeAmnt · BaseDiscAmnt · BillDiscAmnt · VATAmnt · MiscChargAmnt
--   CommissionAmnt · ResvAmnt1-4  = 0 ทั้ง 4,346 ใบ
--               **ไม่มีใบไหนเป็น NULL และไม่มีใบไหนที่ค่าไม่ใช่ศูนย์**
--               ปล่อยเป็น NULL อันตรายเงียบ ๆ เพราะรายงานที่ SUM ข้ามคอลัมน์เหล่านี้
--               จะได้ผลต่างออกไปโดยไม่มีใครสังเกต
--
-- ตัว procedure คัดลอกจากของจริงบนฐาน (หลัง 084) เปลี่ยนเฉพาะรายการฟิลด์และค่า
-- ไม่แตะโครงสร้าง dbo — เขียนค่าลงคอลัมน์ที่มีอยู่แล้วเท่านั้น
-- =============================================================
