-- =============================================================
-- 088_confirm_so_valid_salesman.sql
--
-- WINSpeed ปฏิเสธใบด้วย "Salesman is not vaid!" ตอนกดอนุมัติ
--
-- ต้นเหตุ
--   sp_ConfirmSalesOrder มีบรรทัด  IF @EmpID IS NULL SET @EmpID = 1000
--   ผู้ที่กดยืนยันซึ่งไม่มี EmpId ผูกไว้ (บทบาท ADMIN · ACCOUNTING · ผู้ดูแล)
--   จะทำให้ใบได้ EmpID = 1000 เสมอ
--
--   EmpID 1000 มีอยู่ใน dbo.EMEmp และไม่ได้ลาออก **แต่ไม่ได้ขึ้นทะเบียนพนักงานขาย**
--   WINSpeed เก็บทะเบียนพนักงานขายไว้ที่ dbo.EMSales แยกต่างหาก (33 คน)
--   และตรวจข้อนี้ตอน Approve & Save
--
--   วัดจากใบ 103 ปี 2569 จำนวน 4,345 ใบ
--     EmpID อยู่ใน EMSales      4,289
--     EmpID ว่าง                    56
--     EmpID อยู่นอกทะเบียน            0   <-- ไม่มีเลยสักใบ
--
--   ผลคือใบที่ผู้ดูแลกดยืนยันแทนพนักงานขาย **อนุมัติไม่ได้ทั้งใบ**
--   และไม่มีสัญญาณใด ๆ จนกว่าจะมีคนไปกดอนุมัติแล้วเจอ error
--
-- วิธีแก้
--   เอา EmpId ของผู้ที่ถูกระบุเป็นเจ้าของใบมาใช้ตามเดิม แต่ถ้าไม่อยู่ใน EMSales
--   ให้เป็น NULL แทนการเดา · WINSpeed ยอมรับค่าว่าง และผู้อนุมัติเลือกพนักงานขาย
--   บนหน้าจอได้เอง ซึ่งดีกว่าการยัดคนผิดเข้าไปเงียบ ๆ
--
-- ยังค้าง (แยกเป็นอีกงาน)
--   หน้าจอแอปควรเตือนตั้งแต่ตอนสร้างใบว่ายังไม่ได้ระบุพนักงานขายที่ขึ้นทะเบียน
--   ไม่ใช่ปล่อยให้ไปตายที่ขั้นอนุมัติใน WINSpeed
--
-- ตัว procedure คัดลอกจากของจริงบนฐาน (หลัง 087) เปลี่ยนเฉพาะบล็อกเลือก @EmpID
-- =============================================================
