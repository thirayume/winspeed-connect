...
-- เดิมบรรทัดนี้เป็น `USE dbwins_worldfert9;` ซึ่งเป็นข้อบกพร่องร้ายแรง:
-- รัน migration ชุดนี้กับฐานชื่ออื่น (เช่น dbwins_worldfert9_test) แล้วบรรทัดนี้
-- จะสลับ session ไปฐาน production คำสั่งที่เหลือทั้งไฟล์จึงไปแก้ trigger ผิดฐาน
-- และ ledger ก็ถูกเขียนลงผิดฐานตามไปด้วย
--
-- ตัวรัน (run_migrations.js) เลือกฐานปลายทางให้อยู่แล้ว migration จึงไม่ต้องสลับเอง
-- ตอนนี้ run_migrations.js มีด่านกันไฟล์ที่มี USE ไปฐานอื่นด้วย
SET NOCOUNT ON;
GO
