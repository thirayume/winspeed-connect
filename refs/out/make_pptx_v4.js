...
// ══════════════════════════════════════════════════════════════
// V4 ADDITIONS — Approval/Dashboard · Traceability · Diagrams · UI
// ══════════════════════════════════════════════════════════════
const PNG = "M:\\My Drive\\World Fert\\wf\\out\\diagram_png\\";
const SZ = JSON.parse(require("fs").readFileSync(PNG+"_sizes.json","utf8"));
// fit image into a box (x,y,w,h) preserving aspect, centered
function fitImg(s, file, x, y, w, h){
  const m = SZ[file] || {w:1200,h:700}; const ar = m.w/m.h;
  let iw=w, ih=w/ar; if(ih>h){ ih=h; iw=h*ar; }
  s.addImage({ path: PNG+file, x: x+(w-iw)/2, y: y+(h-ih)/2, w: iw, h: ih });
}
...
// ── Divider: PART 3 ──
{
  ...
  s.addShape(pres.shapes.RECTANGLE, { x:3.6, y:1.55, w:2.8, h:0.05, fill:{color:C.gold}, line:{color:C.gold,width:0} });
  T(s, "PART 3", { x:0, y:1.05, w:10, h:0.4, fontSize:14, color:C.ice2, align:"center", charSpacing:8, margin:0 });
  T(s, "Production Package — v4.0", { x:0, y:1.7, w:10, h:0.7, fontSize:30, bold:true, color:C.white, align:"center", margin:0 });
  T(s, "Approval & Dashboard · Traceability · Diagrams (drawio) · UI/UX ทุกหน้า", { x:0, y:2.6, w:10, h:0.4, fontSize:14, color:C.ice, align:"center", margin:0 });
}

// ── Slide: Approval Levels + Dashboard ──
{
  const s = pres.addSlide(); s.background = { color: C.off };
  header(s, "Approval Levels + Dashboard เฉพาะ (จากทีมขาย)");
  card(s, 0.25, 0.75, 5.7, 4.65);
  pill(s, 0.35, 0.82, 2.4, 0.32, "Approval Levels เฉพาะกรณี", C.navy, C.white);
  const ap = [
    ["แก้ไขตอน Picking","คุณสุรชัย คนเดียว","ApproveFlagPicking"],
    ["ขายเกินราคาตั้ง","คุณรุ่งนิรันดร์ (1)","ApproveFlag SO"],
    ["ขายต่ำกว่าตั้ง ≤500","ผจก. 3 ท่าน","ApproveFlag SO"],
    ["ราคา/ต้นทุน","field-level (กรรมการดูต้นทุน)","RBAC"],
  ];
  ap.forEach(([k,v,f],i)=>{ const y=1.3+i*0.92;
    s.addShape(pres.shapes.RECTANGLE, { x:0.35, y, w:5.5, h:0.82, fill:{color:i%2?C.row:C.white}, line:{color:"E5E7EB",width:0.5} });
    T(s, k, { x:0.45, y:y+0.05, w:5.3, h:0.26, fontSize:11, bold:true, color:C.navy, margin:0 });
    T(s, "→ "+v, { x:0.55, y:y+0.31, w:5.3, h:0.24, fontSize:10, color:C.dark, margin:0 });
    T(s, f, { x:0.55, y:y+0.54, w:5.3, h:0.22, fontSize:8.5, color:C.gray, fontFace:MONO, margin:0 });
  });
  card(s, 6.1, 0.75, 3.65, 4.65);
  pill(s, 6.2, 0.82, 2.2, 0.32, "Dashboard (real-time)", C.navy2, C.white);
  const db = [
    ["ตั๋วคงค้าง + aging","> 30 วัน เหลือง · > 45 วัน แดง"],
    ["สถานะรถ (3)","มาถึง-รอคิว / กำลังรับ / ออกแล้ว"],
    ["สต๊อก/ของแถม/กระสอบ","StockFlag=Y · GroupID NULL · 1000"],
    ["Historical autocomplete","ทะเบียน 'ก' → dropdown · สูตรใหม่=แดง"],
    ["Rebate คงเหลือ","ดูอย่างเดียว · รับจากโม่/สต๊อก"],
  ];
  db.forEach(([k,v],i)=>{ const y=1.3+i*0.78;
    T(s, "● "+k, { x:6.22, y, w:3.4, h:0.26, fontSize:10.5, bold:true, color:C.navy, margin:0 });
    T(s, v, { x:6.42, y:y+0.26, w:3.3, h:0.4, fontSize:9, color:C.dark, margin:0, valign:"top" });
  });
}
...
{
  const s = pres.addSlide(); s.background = { color: C.off };
  header(s, "Traceability — Requirement ↔ FR (ครบทุกข้อ)");
  const rows = [
    ["ปัญหาฯ #1 ตันแม่-ลูก","FR-019"],["ปัญหาฯ #2 รายงานชุดตั๋ว","FR-021"],
    ["ปัญหาฯ #3 ของแถม+สต็อก","FR-020"],["ปัญหาฯ #4 รับเฉพาะชุด","FR-021"],
    ["ปัญหาฯ #5 ตรวจซ้ำ","FR-022"],["Rebate FIFO แยก Sales","FR-008/9/10/11"],
    ["บันทึกบัญชี WINSpeed เดิม","D-03 (B)"],["ตั๋ว aging 30/45 วัน","FR-017"],
    ["สถานะรถ 3 สถานะ","FR-017+WeighTicket"],["Approval เฉพาะคน","FR-006/007 (5.5)"],
    ["9 ปัญหา drawio","F1-F8"],["Master: StockFlag/GroupID","§8.3 + DB จริง"],
  ];
  rows.forEach((r,i)=>{ const col=Math.floor(i/6), row=i%6; const x=0.3+col*4.85, y=0.95+row*0.72;
    s.addShape(pres.shapes.RECTANGLE, { x, y, w:4.7, h:0.62, fill:{color:C.white}, line:{color:"D5DEEC",width:1}, shadow:sh() });
    s.addShape(pres.shapes.OVAL, { x:x+0.12, y:y+0.17, w:0.28, h:0.28, fill:{color:C.grnbd}, line:{width:0} });
    T(s, "✓", { x:x+0.12, y:y+0.17, w:0.28, h:0.28, fontSize:11, bold:true, color:C.white, align:"center", valign:"middle", margin:0 });
    T(s, r[0], { x:x+0.5, y:y+0.06, w:2.7, h:0.5, fontSize:9.5, color:C.dark, valign:"middle", margin:0 });
    pill(s, x+3.25, y+0.16, 1.35, 0.3, r[1], C.navy, C.white);
  });
}

// ── Diagram showcase slides (2 per slide) ──
const diagPairs = [
  [["dg_d01.trim.png","Swimlane AS-IS"],["dg_d02.trim.png","Swimlane TO-BE"]],
  [["dg_d03.trim.png","Workflow (Document Flow)"],["dg_d04.trim.png","Work Process (State Machine)"]],
  [["dg_d05.trim.png","DFD (Context + L1)"],["dg_d06.trim.png","ER Diagram (schema wf)"]],
  [["dg_d07.trim.png","UML Use Case"],["dg_d08.trim.png","UML Class"]],
];
diagPairs.forEach(pair=>{
  const s = pres.addSlide(); s.background = { color: C.off };
  header(s, "แผนภาพระบบ (Diagrams) — แก้ไขได้ใน WorldFert_Diagrams_v4.drawio");
  pair.forEach((d,i)=>{ const x=0.2+i*4.9;
    T(s, d[1], { x, y:0.7, w:4.8, h:0.3, fontSize:11, bold:true, color:C.navy, align:"center", margin:0 });
    s.addShape(pres.shapes.RECTANGLE, { x, y:1.05, w:4.8, h:4.3, fill:{color:C.white}, line:{color:"D5DEEC",width:1}, shadow:sh() });
    fitImg(s, d[0], x+0.1, 1.15, 4.6, 4.1);
  });
});

// ── UI showcase slides (4 per slide, 3 slides) ──
const uiAll = [
  ["ui_login.trim.png","Login"],["ui_dashboard.trim.png","Dashboard"],["ui_sale-order.trim.png","ใบสั่งขาย"],["ui_quotation.trim.png","ใบเสนอราคา"],
  ["ui_rebate-pool.trim.png","Rebate Pool"],["ui_rebate-claim.trim.png","Rebate Claim"],["ui_giveaway.trim.png","ของแถม"],["ui_control-ticket.trim.png","ชุดตั๋วคุม"],
  ["ui_paper-trail.trim.png","Paper Trail"],["ui_approvals.trim.png","อนุมัติ/Unlock"],["ui_reports.trim.png","รายงาน"],["ui_admin.trim.png","ผู้ใช้/สิทธิ์"],
];
for(let p=0;p<3;p++){
  const s = pres.addSlide(); s.background = { color: C.off };
  header(s, "UI/UX Design — React + Tailwind + shadcn/ui ("+(p+1)+"/3)");
  for(let i=0;i<4;i++){ const idx=p*4+i; const it=uiAll[idx]; if(!it) continue;
    const col=i%2, row=Math.floor(i/2); const x=0.25+col*4.85, y=0.72+row*2.42;
    T(s, it[1], { x, y, w:4.7, h:0.26, fontSize:10, bold:true, color:C.navy, margin:0 });
    s.addShape(pres.shapes.RECTANGLE, { x, y:y+0.28, w:4.7, h:2.0, fill:{color:C.white}, line:{color:"D5DEEC",width:1}, shadow:sh() });
    fitImg(s, it[0], x+0.06, y+0.34, 4.58, 1.88);
  }
}
...
// ─── Save ──────────────────────────────────────────────────────
const outPath = "M:\\My Drive\\World Fert\\WorldFert_Presentation_v4.pptx";
