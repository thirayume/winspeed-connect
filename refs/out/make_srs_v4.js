// ============================================================
// WorldFert SRS v4.0 — Production Edition (Diagrams + UI/UX + Traceability)
...
const IMGDIR = "M:\\My Drive\\World Fert\\wf\\out\\diagram_png\\";
...
children.push(makeTable(
  ...
  [
    ...
    [{text:"Version",bold:true},{text:"4.0 (Production — Diagrams + UI/UX + Traceability)",bold:true,color:NAVY}],
    ["Date","6 มิถุนายน 2569 (June 6, 2026)"],
...
children.push(makeTable(
  ...
  [
    ...
    [{text:"4.0",bold:true,color:NAVY},{text:"10/06/2569",bold:true},{text:"T. Meeriksom",bold:true},
     {text:"Production Edition: เพิ่มผัง (Swimlane AS-IS/TO-BE, Workflow, Work Process, DFD, ER, UML) + UI/UX ทุกหน้า (React/Tailwind/shadcn) + Approval Levels + Dashboard + Traceability (req↔FR) — ตรวจสอบกับเอกสารทีมครบ (Requirements/drawio/WS Comment)",bold:true,color:NAVY}],
  ],
...
children.push(body([
  run("2-tier เรียบง่าย: React SPA (tablet-first) → NestJS API → "),
...
children.push(makeTable(
  ...
  [
    ["Frontend","React + Vite + TypeScript + Tailwind + shadcn/ui","tablet-first iPad"],
    ["Backend","NestJS (Node 20 LTS) + REST API","JWT auth (Passport)"],
    [{text:"Database",bold:true,color:NAVY},{text:"SQL Server 2022 เท่านั้น (dbwins_worldfert9)",bold:true},"schema wf + dbo"],
    ["ORM / Query","Prisma (SQL Server provider) หรือ mssql driver","ไม่มี DB อื่น"],
    ["Realtime","WebSocket node เดียว (Socket.IO ไม่มี Redis) หรือ polling","แจ้ง Unlock"],
...
children.push(new Paragraph({ alignment:AlignmentType.CENTER, spacing:{ before:60 },
  children:[new TextRun({ text:"Software Requirements Specification | World Fert Co., Ltd. | v4.0 (Production) | 10 มิ.ย. 2569", font:FONT, size:16, color:GRAYTX })] }));

// ═══════════════════════════════════════════════════════════
// ASSEMBLE DOCUMENT
// ═══════════════════════════════════════════════════════════
const doc = new Document({
  creator:"World Fert", title:"WF-SRS-001 v4.0",
  ...
  sections:[{
    ...
    footers:{ default: new Footer({ children:[ new Paragraph({ alignment:AlignmentType.CENTER,
      children:[ new TextRun({ text:"WF-SRS-001 v4.0  ·  หน้า ", font:FONT, size:16, color:GRAYTX }),
...
Packer.toBuffer(doc).then(buf=>{
  fs.writeFileSync("M:\\My Drive\\World Fert\\WorldFert_SRS_v4_0.docx", buf);
  console.log("✅ Saved WorldFert_SRS_v4_0.docx ("+buf.length+" bytes)");
