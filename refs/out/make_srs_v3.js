// ============================================================
// WorldFert SRS v3.0 — Design-Focused, Field-Verified
// CI: Minimal Navy  |  Font: Prompt
// ตัด Budget & MA ออก, เน้นการออกแบบ
// ============================================================
const fs = require("fs");
const {
...
children.push(makeTable(
  ...
  [
    ...
    [{text:"Version",bold:true},{text:"3.0 (Design-Focused / Field-Verified)",bold:true,color:NAVY}],
...
children.push(makeTable(
  ...
  [
    ...
    [{text:"3.0",bold:true,color:NAVY},{text:"06/06/2569",bold:true},{text:"T. Meeriksom",bold:true},
     {text:"เน้นการออกแบบ + ตรวจสอบกับข้อมูลจริง: เพิ่มหมวด Document Flow & State (DocuType/Join Keys/Scenarios), แก้แหล่งข้อมูล (Price=EMSetPriceDT, Stock/ICStock ว่าง, Credit ไม่มีใน WS), CN/DN=DocuType 109/110, ตัดหมวด Budget & MA ออก",bold:true,color:NAVY}],
  ],
  [1100, 1500, 1900, 4860], { fontSize:16 }
));
...
children.push(new Paragraph({ alignment:AlignmentType.CENTER, spacing:{ before:60 },
  children:[new TextRun({ text:"Software Requirements Specification | World Fert Co., Ltd. | v3.0 | 6 มิ.ย. 2569", font:FONT, size:16, color:GRAYTX })] }));

// ═══════════════════════════════════════════════════════════
// ASSEMBLE DOCUMENT
// ═══════════════════════════════════════════════════════════
const doc = new Document({
  creator:"World Fert", title:"WF-SRS-001 v3.0",
  ...
  sections:[{
    ...
    footers:{ default: new Footer({ children:[ new Paragraph({ alignment:AlignmentType.CENTER,
      children:[ new TextRun({ text:"WF-SRS-001 v3.0  ·  หน้า ", font:FONT, size:16, color:GRAYTX }),
...
Packer.toBuffer(doc).then(buf=>{
  fs.writeFileSync("M:\\My Drive\\World Fert\\WorldFert_SRS_v3_0.docx", buf);
  console.log("✅ Saved WorldFert_SRS_v3_0.docx ("+buf.length+" bytes)");
