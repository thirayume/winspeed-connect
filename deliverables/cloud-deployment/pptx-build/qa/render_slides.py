...
def _render_presentation_with_artifact_tool(
    ...
    # On the bundled Windows runtime the renderer can finish every PNG and
    # print valid JSON, then terminate with 0xC0000409 during native teardown.
    # Treat that specific post-render crash as success only when every output
    # declared by the JSON payload exists; all other non-zero exits still fail.
    payload = None
    ...
    if proc.returncode != 0:
        declared = payload.get("paths", []) if isinstance(payload, dict) else []
        if not declared or not all(exists(p) for p in declared):
