...
.\tunnel.ps1 -ServerIp YOUR_IP -KeyFile $HOME\.ssh\id_ed25519
```
...
**DBeaver (MySQL)** — ทางที่สะดวกกว่าคือใช้ SSH tunnel ในตัว DBeaver:
| แท็บ | ช่อง | ค่า |
|---|---|---|
| Main | Host / Port | `localhost` / `3306` |
| Main | Database | `db_truckscale` |
| Main | User / Password | `wfapp` / รหัสจากขั้น 3 |
| **SSH** | ☑ Use SSH Tunnel · Host / User | `YOUR_IP` / `root` |
| SSH | Auth method | Public key → `id_ed25519` |

> ใช้ SSH tunnel ในตัว DBeaver แล้ว **ไม่ต้องรัน tunnel.ps1** สำหรับ MySQL
> (ถ้าใช้ `tunnel.ps1` ให้ต่อที่ `localhost:33060` แทน)

**DBeaver (SQL Server)** ก็ทำแบบเดียวกันได้ — Host `localhost` Port `1433` + แท็บ SSH
